# FormDB Client Libraries

Official client libraries for FormDB - the narrative-first, reversible, audit-grade database.

## Available Clients

| Language | Package | Status | Features |
|----------|---------|--------|----------|
| ReScript | `@formdb/rescript` | Stable | Type-safe, Deno runtime |
| PHP | `formdb/formdb-php` | Stable | PSR-18, Laravel/Symfony |
...
$client = FormDBClient::fromEnv($httpClient, $httpFactory, $httpFactory);
...
let client = fromEnv() // Uses FORMDB_URL, FORMDB_API_KEY
```
...
$client = FormDBClient::fromEnv(...); // Uses FORMDB_URL, FORMDB_API_KEY
```
...
| FormDBError(err) => Console.error(`Error: ${err.message}`)
}
```
