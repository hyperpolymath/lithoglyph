...
use FormDB\Query\QueryBuilder;
use FormDB\Query\InsertBuilder;
use FormDB\Query\UpdateBuilder;
use FormDB\Query\DeleteBuilder;
use FormDB\Types\Collection;
use FormDB\Types\CollectionType;
use FormDB\Types\FunctionalDependency;
use FormDB\Types\HealthResponse;
use FormDB\Types\JournalEntry;
use FormDB\Types\MigrationStatus;
use FormDB\Types\NormalForm;
use FormDB\Types\NormalFormAnalysis;
use FormDB\Types\Provenance;
use FormDB\Types\QueryResult;
 ...
 * FormDB Exception
 */
class FormDBException extends \Exception
{
 ...
 * FormDB PHP Client
 *
 * Type-safe client for FormDB REST API
 */
final class FormDBClient
{
    ...
    public static function fromEnv(
        ...
        $baseUrl = getenv('FORMDB_URL') ?: 'http://localhost:8080';
        $client = new self($baseUrl, $httpClient, $requestFactory, $streamFactory);

        if ($apiKey = getenv('FORMDB_API_KEY')) {
