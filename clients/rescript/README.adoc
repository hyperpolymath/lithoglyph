...
The client reads `+LITHOGLYPH_URL+` / `+LITHOGLYPH_API_KEY+` environment
variables, falling back to the legacy `+LITH_URL+` / `+LITH_API_KEY+`
names.

The old `+Lith+` / `+Lith_Types+` / `+Lith_Query+` modules are still
present for backwards compatibility but should be considered deprecated.
