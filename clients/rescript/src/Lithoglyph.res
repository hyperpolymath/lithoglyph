 ...
 *  Reads LITHOGLYPH_URL (or FORMDB_URL for backwards compatibility)
 *  and LITHOGLYPH_API_KEY (or FORMDB_API_KEY). */
let fromEnv = () => {
  let baseUrl = %raw(`
    (typeof Deno !== 'undefined')
      ? (Deno.env.get("LITHOGLYPH_URL") || Deno.env.get("FORMDB_URL") || "http://localhost:8080")
      : "http://localhost:8080"
  `)
  let apiKey = %raw(`
    (typeof Deno !== 'undefined')
      ? (Deno.env.get("LITHOGLYPH_API_KEY") || Deno.env.get("FORMDB_API_KEY") || null)
      : null
  `)
