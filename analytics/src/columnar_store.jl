...
"""
    sync!(store::ColumnarStore, client::FormBDClient, collection::String; mode::Symbol=:full)

Sync data from FormBD to columnar store.
Modes: :full (replace all), :incremental (append new)
"""
function sync!(store::ColumnarStore, client::FormBDClient, collection::String; mode::Symbol=:full)
    start_time = now()
