...
"""
HTTP API for FormBD-Analytics using Oxygen.jl
"""
...
function serve(config::Config)
    # Initialize state
    client = FormBDClient(config.formbd.api_url)
    store = ColumnarStore(config.storage.data_dir)

    # Load existing data
    for collection in config.formbd.collections
        load!(store, collection)
    end

    APP_STATE[] = AppState(client, store, config)

    @info "FormBD-Analytics starting" host=config.server.host port=config.server.port

    # Define routes
    @get "/analytics/health" function()
        state = APP_STATE[]
        formbd_reachable = health_check(state.client)

        return Dict(
            "status" => "ok",
            "formbd_reachable" => formbd_reachable,
