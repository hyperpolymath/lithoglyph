...
struct FormBDClient
    ...
    function FormBDClient(base_url::String; timeout::Int=30)
...
"""
Document from FormBD with provenance
"""
struct Document
    id::String
...
"""
    fetch_collection(client::FormBDClient, collection::String) -> Vector{Document}

Fetch all documents from a FormBD collection.
"""
function fetch_collection(client::FormBDClient, collection::String)::Vector{Document}
    url = "$(client.base_url)/collections/$collection/documents"

    @info "Fetching documents from FormBD" url

    response = HTTP.get(url; readtimeout=client.timeout)

    if response.status != 200
        error("FormBD API error: $(response.status)")
    end
...
"""
    fetch_document(client::FormBDClient, collection::String, id::String) -> Document

Fetch a single document by ID.
"""
function fetch_document(client::FormBDClient, collection::String, id::String)::Document
...
"""
    health_check(client::FormBDClient) -> Bool

Check if FormBD API is reachable.
"""
function health_check(client::FormBDClient)::Bool
    try
