...
This tool generates type-safe client libraries from the Lith API
specification. It supports multiple target languages and produces
...
[arabic]
. Create `+<Language>Gen.res+` implementing the generator
. Add case to `+Main.res+` switch statement
. Add task to `+deno.json+`

=== Hand-Crafted vs Generated

The SDK generator produces basic clients. The hand-crafted clients in
`+clients/+` include:
...
Use the generator for: - Quick prototyping - Keeping types in sync -
