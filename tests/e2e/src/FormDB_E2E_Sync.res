...
/**
 * FormDB E2E Sync Tests
 *
 * End-to-end tests for CMS sync scenarios
 */

open FormDB_E2E_Types
...
let test_bidirectionalSync = async (env: testEnvironment): e2eResult => {
  ...
  try {
    ...
    // Create from "FormDB" (simulated)
    let createBody2 = Js.Json.object_(Js.Dict.fromArray([
      ("fdql", Js.Json.string(`INSERT INTO ${collectionName} {"id": "bi-2", "title": "From FormDB", "source": "formdb"}`)),
    ]))
