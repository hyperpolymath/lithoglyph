...
/**
 * FormDB Strapi Integration Tests
 *
...
let test_syncOnCreate = async (): testResult => {
  ...
  assertEqual(Array.length(requests), 1, "Should make one request to FormDB")
}
...
let test_syncModeBidirectional = async (): testResult => {
  ...
  let shouldSyncToCms = syncMode == "bidirectional" || syncMode == "formdb-to-cms"
  let shouldSyncToFormDB = syncMode == "bidirectional" || syncMode == "cms-to-formdb"

  if shouldSyncToCms && shouldSyncToFormDB {
    Passed
  } else {
...
/** Test: Sync mode - one-way to FormDB */
let test_syncModeCmsToFormDB = async (): testResult => {
  let syncMode = "cms-to-formdb"
  let shouldSyncToFormDB = syncMode == "bidirectional" || syncMode == "cms-to-formdb"
  let shouldSyncToCms = syncMode == "bidirectional" || syncMode == "formdb-to-cms"

  if shouldSyncToFormDB && !shouldSyncToCms {
    Passed
  } else {
    Failed({message: "cms-to-formdb should only sync to FormDB"})
  }
}
...
let runStrapiTests = async (): {passed: int, failed: int, skipped: int} => {
  ...
  let tests = [
    ...
    ("Sync mode - cms-to-formdb", test_syncModeCmsToFormDB),
