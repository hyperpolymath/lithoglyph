...
/**
 * FormDB Ghost Integration Tests
 *
...
let test_postPublishedEvent = async (): testResult => {
  ...
  assertEqual(Array.length(requests), 1, "Should sync post to FormDB")
}
...
let test_postUpdatedEvent = async (): testResult => {
  ...
  let requests = getRequests(client.requestStore)
  assertEqual(Array.length(requests), 1, "Should update post in FormDB")
}
...
let test_postDeletedEvent = async (): testResult => {
  ...
  let requests = getRequests(client.requestStore)
  assertEqual(Array.length(requests), 1, "Should delete post from FormDB")
}
