...
/**
 * FormDB Payload CMS Integration Tests
 *
 * Tests for Payload CMS adapter functionality
 */

open FormDB_Integration_Types
open FormDB_Integration_Mock

/** Test: Plugin configuration */
let test_pluginConfiguration = async (): testResult => {
  let config = Js.Dict.fromArray([
    ("formdbUrl", Js.Json.string("http://localhost:8080")),
    ("enabled", Js.Json.boolean(true)),
    ("collections", Js.Json.array([])),
  ])

  let hasUrl = Js.Dict.get(config, "formdbUrl")->Option.isSome
...
let test_collectionMapping = async (): testResult => {
  let mapping = Js.Dict.fromArray([
    ...
    ("formdbCollection", Js.Json.string("posts")),
  ...
  let hasFormdbCollection = Js.Dict.get(mapping, "formdbCollection")->Option.isSome
  let hasSyncMode = Js.Dict.get(mapping, "syncMode")->Option.isSome

  assertTrue(hasPayloadSlug && hasFormdbCollection && hasSyncMode, "Should have all mapping fields")
}

/** Test: afterChange hook - create */
let test_afterChangeCreate = async (): testResult => {
  let client = makeMockFormDBClient()
...
let test_syncModeFiltering = async (): testResult => {
  let shouldSyncToFormDB = (mode: string): bool => {
    mode == "bidirectional" || mode == "payload-to-formdb"
  }

  let shouldSyncToPayload = (mode: string): bool => {
    mode == "bidirectional" || mode == "formdb-to-payload"
  }

  let bi = shouldSyncToFormDB("bidirectional") && shouldSyncToPayload("bidirectional")
  let ptf = shouldSyncToFormDB("payload-to-formdb") && !shouldSyncToPayload("payload-to-formdb")
  let ftp = !shouldSyncToFormDB("formdb-to-payload") && shouldSyncToPayload("formdb-to-payload")
