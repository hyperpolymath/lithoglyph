...
/**
 * FormDB Directus Integration Tests
 *
...
let test_envConfiguration = async (): testResult => {
  let config = Js.Dict.fromArray([
    ...
    ("FORMDB_API_KEY", "test-key"),
    ("FORMDB_SYNC_COLLECTIONS", "articles,products,users"),
  ])

  let hasUrl = Js.Dict.get(config, "FORMDB_URL")->Option.isSome
  let hasSyncCollections = Js.Dict.get(config, "FORMDB_SYNC_COLLECTIONS")->Option.isSome
