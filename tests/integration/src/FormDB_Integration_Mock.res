...
/** Mock FormDB client for testing */
type mockFormDBClient = {
  requestStore: requestStore,
  mutable responses: array<mockResponse>,
  mutable responseIndex: int,
}

/** Create mock FormDB client */
let makeMockFormDBClient = (): mockFormDBClient => {
  {
...
let addResponse = (client: mockFormDBClient, response: mockResponse): unit => {
  client.responses->Array.push(response)->ignore
}

/** Get next response */
let nextResponse = (client: mockFormDBClient): mockResponse => {
