...
[.lead]
Specification for FormDB's fixed-size block storage format.
...
FormDB stores all data in fixed-size blocks. Each block has a header that provides:
...
When validation fails, FormDB provides structured guidance:
