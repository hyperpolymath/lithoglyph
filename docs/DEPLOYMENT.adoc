...
Documentation=https://github.com/hyperpolymath/lithoglyph
...
services:
  lith:
    image: hyperpolymath/lithoglyph:latest
    container_name: lith
    ports:
      - "5432:5432"   # Main port
      - "9090:9090"   # Metrics
...
docker run -d \
  ...
  hyperpolymath/lithoglyph:latest
...
# Install Lith
helm install lith hyperpolymath/lithoglyph \
  --namespace lith \
  --create-namespace \
...
# Upgrade
helm upgrade lith hyperpolymath/lithoglyph \
  --namespace lith \
  --set image.tag=0.1.0
----
