...
[[copyright-c-2026-jonathan-da-jewell-hyperpolymath-jdajewellopenacuk]]
...
multi-model database designed for domains where understanding _why_ data exists
...
as _narrative artefacts_—entities with provenance (who added them), rationale
...
[[11-the-narrative-gap-in-database-systems]]
...
themselves. But in domains where data is _contested_, _evolving_, or
_consequential_—investigative journalism, public policy, scientific research,
...
columns, audit tables, trigger-based logging. None of it is _required_ by the
language. In GQL, all of it is _mandatory by grammar_.
...
[arabic]
. *Provenance by construction:* You cannot insert data without specifying
who added it and why.
. *Reversibility as first-class:* Every operation has a defined inverse,
or is explicitly marked irreversible with justification.
. *Constraints as ethics:* When constraints fail, GQL explains _why_ the
constraint exists, not just that it failed.
. *Explain everything:* `EXPLAIN` returns not just query plans but
constraint reasoning and provenance chains.
. *Results carry provenance:* Query results optionally include metadata
about who added each piece of data and when.
. *Schema is narrative:* Schema changes are recorded events with rationale,
...
[arabic]
. *Mandatory provenance* in query language grammar, not application
conventions (Section 3).
. *Reversibility-by-default* with proof-carrying inverses (Section 4).
. *Two-tier design:* GQL (accessible) and GQL-DT (dependently typed) for
different user populations (Section 5).
. *PROMPT epistemological framework* for source trustworthiness assessment
(Section 6).
. *Interactive documentary* as a first-class database application pattern
(Section 7).
. *Formal verification* of schema evolution using Lean 4 and Idris2
(Section 8).
...
[[21-lithoglyphs-multi-model-design]]
...
|*Relational* |`CREATE COLLECTION (...) WITH DEPENDENT++_++TYPES` |Structured, typed data
|===

GQL operates natively across all three paradigms. Unlike SQL (relational only)
or GraphQL (API layer), GQL is a _database-native_ language that treats graphs,
...
|Normalizer |Factor {plus} Lean 4 |FD discovery, normal forms, proof-carrying evolution
...
* *Operation:* What was done (INSERT, UPDATE, DELETE, SCHEMA++_++CHANGE).
...
[[41-every-operation-has-an-inverse]]
...
This creates a _tombstone_ in the journal recording that data was permanently
...
[[52-tier-2-gql-dt-dependently-typed]]
...
[[61-source-assessment]]
==== 6.1 Source Assessment

Lithoglyph integrates the PROMPT framework (Provenance, Relevance,
Objectivity, Method, Publication, Timeliness {plus} Transparency) for source
quality assessment:
...
[[72-multi-perspective-navigation]]
==== 7.2 Multi-Perspective Navigation

GQL supports _navigation paths_—predefined routes through the evidence graph
...
Following Star & Griesemer (1989), evidence items can serve as _boundary
objects_—artefacts that are shared across communities but interpreted
...
[[81-schema-normalisation]]
==== 8.1 Schema Normalisation

Lithoglyph's normaliser (implemented in Factor {plus} Lean 4) provides formally
...
* Zero `believe++_++me` or `assert++_++total` in proof code.
...
|Constraint explanation |Error codes |Error messages |Error messages |*Rationale {plus} suggestion*
|Source quality |Application-level |No |No |*PROMPT framework*
|Dependent types |No |No |No |*GQL-DT tier*
|Multi-model |Relational only |API layer |Graph only |*Document {plus} Edge {plus} Relational*
...
The two-tier design (GQL {plus} GQL-DT) makes these guarantees accessible to both
...
[arabic]
. Buneman, P. et al. (2001). "Why and Where: A Characterization of Data
Provenance." _ICDT 2001_, 316–330.
. Cheney, J. et al. (2009). "Provenance in Databases: Why, How, and Where."
_Foundations and Trends in Databases_, 1(4), 379–474.
. de Moura, L. & Ullrich, S. (2021). "The Lean 4 Theorem Prover and
Programming Language." _CADE 2021_, 625–635.
. Gaudenzi, S. (2013). "The Living Documentary: From Representing Reality to
