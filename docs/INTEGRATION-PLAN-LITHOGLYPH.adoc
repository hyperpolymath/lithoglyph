...
[[extracted-from-the-master-integration-plan-bofigdocsintegration-planmd]]
...
|L3 |Evidence collection schema for bofig |High |Small |*COMPLETE* — 5 collections {plus} 5 GQL test vectors
|L4 |Financial transaction collection |High |Small |source, destination, amount, date, instrument, intermediary
|L5 |Entity collection {plus} co-reference resolution |High |Medium |Person/org/location entities with alias tracking
...
* Every mutation has actor {plus} rationale (accountability)
...
==== Integration 1: Docudactyl → Lithoglyph (D2 {plus} L6)
...
==== Integration 2: Lithoglyph → Bofig (L3 {plus} B5)
...
[[integration-3-entity-resolution-loop-d3d4d5--l5--b1]]
...
* *Epstein Lithoglyph phases:* `docs/EPSTEIN-INGEST-TESTS.adoc` (this repo)
