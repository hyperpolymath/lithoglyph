...
* _Hollow_ (Elaine McMillion Sheldon): Multi-perspective rural documentary
* _Fort McMoney_ (David Dufresne): Interactive journalism game
* _Gaza/Sderot_ (Bruno {plus} Arte): Dual-perspective conflict coverage

[[12-the-database-problem]]
==== 1.2 The Database Problem

i-docs require databases that can:

[arabic]
. *Track evidence provenance*: "Where did this claim come from?"
. *Support corrections transparently*: Retractions, updates, clarifications
. *Enable multi-perspective navigation*: Same evidence, different audience paths
. *Maintain epistemological metadata*: Quality scores, verification status
. *Preserve narrative context*: Why was this evidence included? By whom? When?
...
[[13-lith-as-i-doc-infrastructure]]
...
[[22-boundary-objects-star--griesemer-1989]]
...
[[23-epistemological-transparency]]
...
[[53-corrections-and-retractions]]
...
[[74-conflict-resolution-narrative]]
...
** *Metric*: `SELECT AVG(prompt++_++overall) FROM evidence`
* *Verification Latency*: Time from claim to verification
** *Target*: ++<++48 hours
** *Metric*: `SELECT AVG(verified++_++at - added++_++at) FROM claims WHERE verified++_++by IS NOT NULL`

[[92-reader-engagement]]
==== 9.2 Reader Engagement

* *Navigation Path Completion*: % of readers who complete path
** *Target*: ++>++60%
** *Tracked via*: Frontend analytics {plus} Lith queries
...
*Lith Advantage*: Combines evidence management {plus} epistemology {plus} narrative.
...
[arabic]
. *i-docs at scale*: Navigation over narration, reader agency
. *Transparent epistemology*: PROMPT scores, evidence chains
. *Correction workflows*: Retractions with full context
. *Multi-agent verification*: Dempster-Shafer fusion audit trails
. *Boundary objects*: Same evidence, multiple audience perspectives
...
[arabic]
. Migrate BoFIG from ArangoDB to Lith (Month 1-3)
. User testing with NUJ journalists (Month 3, 6, 12)
. My-Newsroom integration (Month 7-12)
. Full-scale Reuters-level newsroom (Month 19-24)

*Impact*: Lith {plus} BoFIG {plus} My-Newsroom could become the *de facto standard* for i-doc journalism, much as Zotero became the standard for academic reference management.

'''''

*Document Status*: Living document. Updates tracked in `spec/WP05++_++CHANGELOG.md`.

*See Also*:

* https://github.com/Hyperpolymath/bofig[BoFIG Repository]
* https://github.com/hyperpolymath/my-newsroom[My-Newsroom Repository]
* https://opendoclab.mit.edu/[MIT Open Doc Lab]
* https://example.com/prompt-framework[PROMPT Framework]
