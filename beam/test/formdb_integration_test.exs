...
#
# Integration tests for the FormDB BEAM NIF pipeline.
#
# These tests exercise the full lifecycle of FormDB operations through the
...
# (beam/native/src/formdb_nif.zig) correctly bridges BEAM to the Lithoglyph
# storage engine via the FormBD C ABI (generated/abi/bridge.h).
#
...
#
# Architecture:
#   Elixir/ExUnit -> formdb_nif.erl -> formdb_nif.zig (NIF) -> FormBD C ABI
#                                                                    |
#                                                        core-zig/src/blocks.zig
#                                                        (4 KiB blocks, CRC32C)

defmodule FormdbIntegrationTest do
  ...
  describe "full lifecycle: insert, commit, verify" do
    test "open -> begin txn -> apply insert -> commit -> close", %{db_path: db_path} do
      ...
      result = :formdb_nif.apply(txn_ref, cbor_document)
    ...
    test "multiple inserts within a single transaction", %{db_path: db_path} do
      ...
      result1 = :formdb_nif.apply(txn_ref, doc1)
      ...
      result2 = :formdb_nif.apply(txn_ref, doc2)
      ...
      result3 = :formdb_nif.apply(txn_ref, doc3)
    ...
    test "insert and verify block IDs are unique", %{db_path: db_path} do
      ...
      block_id_1 = extract_block_id(:formdb_nif.apply(txn_ref, doc1))
      block_id_2 = extract_block_id(:formdb_nif.apply(txn_ref, doc2))
  ...
  describe "transaction abort (rollback)" do
    test "abort discards all pending operations", %{db_path: db_path} do
      ...
      assert {:ok, schema} = :formdb_nif.schema(db_ref)
      assert schema == <<0xA0>>
  ...
  describe "read-only transactions" do
    test "read-only transaction can read schema", %{db_path: db_path} do
      {:ok, db_ref} = :formdb_nif.db_open(db_path)
      {:ok, txn_ref} = :formdb_nif.txn_begin(db_ref, :read_only)
    ...
    test "read-only transaction can read journal", %{db_path: db_path} do
      ...
      assert {:ok, journal} = :formdb_nif.journal(db_ref, 0)
  ...
  describe "database persistence across open/close" do
    test "open, write, close, reopen succeeds", %{db_path: db_path} do
      ...
      assert {:ok, db_ref2} = :formdb_nif.db_open(db_path)
      refute is_nil(db_ref2)

      # Should be able to read schema from reopened database
      assert {:ok, _schema} = :formdb_nif.schema(db_ref2)

      :formdb_nif.db_close(db_ref2)
    end
  end
  ...
  describe "CBOR payload handling" do
    test "accepts minimal CBOR map (empty document)", %{db_path: db_path} do
      ...
      # 0xA0 = empty CBOR map {}
      result = :formdb_nif.apply(txn_ref, <<0xA0>>)
      assert_apply_success(result)
    ...
    test "accepts CBOR with Lithoglyph PROMPT score tag (39006)", %{db_path: db_path} do
      ...
      result = :formdb_nif.apply(txn_ref, cbor_with_tag)
      assert_apply_success(result)
    ...
    test "accepts CBOR with provenance tag (39004) and nested actor tag (39005)", %{db_path: db_path} do
      ...
      cbor_with_provenance = <<
        ...
        0x64, "test"
      >>
      result = :formdb_nif.apply(txn_ref, cbor_with_provenance)
      assert_apply_success(result)
    ...
    test "rejects payload exceeding 1 MiB limit", %{db_path: db_path} do
      ...
      # 1 MiB + 1 byte exceeds the formbd_parse_cbor limit
  ...
  describe "sequential transactions" do
    test "multiple sequential transactions on same database", %{db_path: db_path} do
      ...
      {:ok, txn1} = :formdb_nif.txn_begin(db_ref, :read_write)
      doc1 = <<0xA1, 0x64, "data", 0x62, "t1">>
      :formdb_nif.apply(txn1, doc1)
      assert :ok = :formdb_nif.txn_commit(txn1)

      # Transaction 2: insert
      {:ok, txn2} = :formdb_nif.txn_begin(db_ref, :read_write)
      doc2 = <<0xA1, 0x64, "data", 0x62, "t2">>
      :formdb_nif.apply(txn2, doc2)
      assert :ok = :formdb_nif.txn_commit(txn2)

      # Transaction 3: read-only
      {:ok, txn3} = :formdb_nif.txn_begin(db_ref, :read_only)
      :formdb_nif.txn_commit(txn3)
  ...
  describe "error recovery" do
    test "database usable after failed apply", %{db_path: db_path} do
      ...
      {:error, _} = :formdb_nif.apply(txn_ref, <<>>)

      # Abort the failed transaction
      :formdb_nif.txn_abort(txn_ref)

      # Start a new transaction - database should still be usable
      {:ok, txn_ref2} = :formdb_nif.txn_begin(db_ref, :read_write)
      doc = <<0xA1, 0x64, "data", 0x62, "ok">>
      result = :formdb_nif.apply(txn_ref2, doc)
      assert_apply_success(result)

      :formdb_nif.txn_commit(txn_ref2)
      :formdb_nif.db_close(db_ref)
    end
    ...
    test "database usable after aborted transaction", %{db_path: db_path} do
      ...
      :formdb_nif.apply(txn1, doc)
      :formdb_nif.txn_abort(txn1)

      # New transaction should work
      {:ok, txn2} = :formdb_nif.txn_begin(db_ref, :read_write)
      doc2 = <<0xA1, 0x64, "data", 0x64, "good">>
      result = :formdb_nif.apply(txn2, doc2)
      assert_apply_success(result)
      :formdb_nif.txn_commit(txn2)
  ...
  describe "evidence document lifecycle" do
    test "insert evidence document with full CBOR structure", %{db_path: db_path} do
      ...
      result = :formdb_nif.apply(txn_ref, evidence_cbor)
      assert_apply_success(result)
      assert :ok = :formdb_nif.txn_commit(txn_ref)

      # Verify journal has entries (will have content once real storage is active)
      {:ok, journal} = :formdb_nif.journal(db_ref, 0)
      assert is_binary(journal)
