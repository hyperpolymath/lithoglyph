...
// ============================================================
// FormDB C ABI declarations (from generated/abi/bridge.h)
//
// These extern functions are provided by libformbd.so (the Zig FFI
// bridge implementation at ffi/zig/src/bridge.zig). The Rust NIF
// shared library links against libformbd at load time.
// ============================================================
...
extern "C" {
    ...
    /// Get FormDB version as encoded integer (major * 10000 + minor * 100 + patch).
    fn fdb_version() -> u32;
}
...
// NIF functions
rustler::init!("formdb_nif", load = load);
