...
//
// Configures the linker to find libformbd.so (the core Zig FFI bridge).
// The library path can be set via:
//   - FORMBD_LIB_DIR environment variable
//   - Default: ../../ffi/zig/zig-out/lib (relative to this crate)

fn main() {
    // Allow the user to override the library search path
    let lib_dir = std::env::var("FORMBD_LIB_DIR").unwrap_or_else(|_| {
    ...
    println!("cargo:rustc-link-lib=dylib=formbd");

    // Re-run if the bridge header changes
    println!("cargo:rerun-if-changed=../../generated/abi/bridge.h");
    println!("cargo:rerun-if-env-changed=FORMBD_LIB_DIR");
}
