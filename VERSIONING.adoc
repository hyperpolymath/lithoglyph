...
[.lead]
API stability guarantees, versioning conventions, and compatibility promises for FormDB.

toc::[]

== Overview

FormDB takes stability seriously. As a database designed for long-term archives and audit trails, we understand that your data and queries must remain accessible for years or decades. This document describes our versioning scheme, stability guarantees, and migration policies.

[IMPORTANT]
====
**Current Status: Pre-1.0 Development**

FormDB is currently at version **0.0.1** (conceptual + PoC stage). All APIs, formats, and interfaces are subject to change. Once we reach 1.0, the stability guarantees in this document become binding commitments.
====

== Version Numbering

FormDB follows https://semver.org/[Semantic Versioning 2.0.0]:
...
Each FormDB component is assigned a stability tier that defines its compatibility guarantees.
...
FormDB uses internal version numbers for on-disk formats to ensure data remains readable across upgrades.
...
Journals written by any FormDB version can be replayed by any later version. This is a **permanent guarantee**—we will never break journal replay.
...
[source,text]
----
libformdb.so.1      # ABI version 1
libformdb.so.1.2.3  # Full version
----
...
FormDB provides migration tools for format changes:

[source,bash]
----
# Check if migration is needed
formdb doctor --check-version mydb/

# Backup before migration
formdb backup mydb/ mydb-backup-$(date +%Y%m%d)/

# Run migration
formdb migrate mydb/ --to-version 2

# Verify migration
formdb doctor --verify mydb/
----
...
formdb lint --queries queries/*.fql

# Auto-fix where possible
formdb lint --fix --queries queries/*.fql

# Generate migration report
formdb lint --report queries/*.fql > migration-report.md
----
...
| FQL v1 | FormDB 1.x | Yes
| FQL v1 | FormDB 2.x | Yes (with version directive)
| FQL v1 | FormDB 3.x | Yes (with version directive)
|===

=== Journal Replay Guarantee

**Permanent Commitment**: Any journal written by any FormDB version can be replayed by any later version. This enables:
...
[source,bash]
----
formdb doctor --compatibility mydb/

FormDB Compatibility Report
===========================
Database created by: 0.0.1
Current FormDB version: 0.1.0
