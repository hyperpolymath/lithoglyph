...
/// In production, use FORMDB_JWT_SECRET (read via config) for real JWT validation.
...
pub const AuthError = error{
    ...
    MalformedToken,
    /// JWT secret not configured (FORMDB_JWT_SECRET missing)
...
pub fn validateJWT(token: []const u8) bool {
    ...
    if (cfg.jwt_secret == null) {
        log.warn("JWT validation failed: FORMDB_JWT_SECRET not set", .{});
        return false;
    }
...
pub fn createJWT(claims: Claims) AuthError![]const u8 {
    ...
    //       3. Sign header.payload with FORMDB_JWT_SECRET using HMAC-SHA256
    //       4. Return header.payload.signature
    //
    //       Until implemented, callers should use API key auth instead.

    const secret = std.posix.getenv("FORMDB_JWT_SECRET") orelse {
        log.err("Cannot create JWT: FORMDB_JWT_SECRET environment variable not set", .{});
...
pub fn parseJWT(token: []const u8) AuthError!Claims {
    ...
    //       3. Verify signature using FORMDB_JWT_SECRET
    ...
    const secret = std.posix.getenv("FORMDB_JWT_SECRET") orelse {
        log.err("Cannot parse JWT: FORMDB_JWT_SECRET environment variable not set", .{});
        return AuthError.JwtSecretNotConfigured;
    };
    _ = secret;
