...
test "block header validation" do
  ...
  for vector <- data["vectors"] do
    if vector["valid"] do
      assert {:ok, _} = FormDB.validate_header(vector["input_fields"])
    else
      assert {:error, _reason} = FormDB.validate_header(vector["input_fields"])
    end
  end
end
```
...
- ABI types: `src/FormBD/FormBridge.idr`, `src/FormBD/FormLayout.idr`
